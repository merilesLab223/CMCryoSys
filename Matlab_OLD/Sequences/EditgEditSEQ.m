...
switch what
    case 'ApplyPulses'
        SEQ = ApplyPulses(SEQ,hObject, eventdata, handles);
        gEditSEQ = SEQ;
...
function SEQ = ApplyPulses(SEQ,hObject, eventdata, handles)
global gTimeStep;
Value = eval(get(handles.DTPulses,'String'));
Value = round(Value/gTimeStep)*gTimeStep;
PBN = get(handles.PBNPulses,'Value')-2;
if PBN < 0 | PBN > 21, disp('Abort: PBN out of range!'); return; end
chn = GetCHN(SEQ,PBN);
if chn == -1, disp(['PB' num2str(PBN) ' not present in SEQ']); end
Type = get(handles.TypePulses,'String');
bTypePulses = get(handles.bTypePulses,'Value');

for Rise = 1:length(SEQ.CHN(chn).DT)
    if ~bTypePulses | strcmp(SEQ.CHN(chn).Type(Rise),Type)
