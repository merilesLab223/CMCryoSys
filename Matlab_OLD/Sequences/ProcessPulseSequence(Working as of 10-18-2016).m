...
% 11 April 2014
% Jonathan Hodges
% jhodges@diamondnt.com
%
% Added 'Type' class with 'Duration' sweep
% Using functions derived from Ed Chen, Quantum Photonics Group, MIT
%
...
function [Sequence] = PulseSequenceToArray(PSeq,SourceFreq)
    ...
    Sequence = zeros(NumChannels,SeqPoints,'int8');
    ...
    for k=1:numel(PSeq.Channels),
        ...
        for l=1:PSeq.Channels(k).NumberOfRises,
            ...
            if PSeq.Channels(k).HWChannel == MicrowaveHardwareChannel,
                PrePhaseDelay = 30; %clock cycles
                PostPhaseDelay = 30; %clock cycles
...
function [] = ProcessTypeClass(PSeq,tempPSeq,jj)
    ...
    switch PSeq.Sweeps(jj).SweepType,
        ...
        case 'Duration'
			% The block of code contained in this case for Duration comes from Ed Chen
			%
            % implemented by Ed, 2014/02/11
            for k=1:size(allTypeRises,1), % fixed length->size(,1) bug 20110314
                ...
                %                 tempPSeq.Channels(chn).RiseTimes(rise+1:end) = tempPSeq.Channels(chn).RiseTimes(rise+1:end) + x(ind(jj));
                if PSeq.Sweeps(jj).SweepShifts == 2, % shift all channels
                    % find all rise times occuring after the
                    % beginning of this sweep
                    riseT = tempPSeq.Channels(chn).RiseTimes(rise);
                    for k=1:numel(tempPSeq.Channels),
